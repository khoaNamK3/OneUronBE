...
namespace OneUron.DAL.Repository.StudyMethodRepo
{
    public class StudyMethodRepository : GenericRepository<StudyMethod> , IStudyMethodRepository
    {
        ...
        {
            return await _dbSet.Include(sm => sm.Method).Include(sm => sm.User).Where(sm => !sm.IsDeleted).ToListAsync();
        }

        public async Task<StudyMethod> GetByIdAsync(Guid id)
        {
            return await _dbSet.Include(sm => sm.Method).Include(sm => sm.User).FirstOrDefaultAsync(sm => sm.Id == id && !sm.IsDeleted);
        }
    }
}
